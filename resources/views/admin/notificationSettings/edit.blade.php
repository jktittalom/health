...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.notificationSetting.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.notification-settings.update", [$notificationSetting->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id" required>
                    @foreach($users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $notificationSetting->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('weekdays') ? 'is-invalid' : '' }}" type="text" name="weekdays" id="weekdays" value="{{ old('weekdays', $notificationSetting->weekdays) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('all_days') ? 'is-invalid' : '' }}" type="number" name="all_days" id="all_days" value="{{ old('all_days', $notificationSetting->all_days) }}" step="1">
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('time') ? 'is-invalid' : '' }}" type="text" name="time" id="time" value="{{ old('time', $notificationSetting->time) }}" required>
                @if($errors->has('time'))
                    <div class="invalid-feedback">
                        {{ $errors->first('time') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('timezone') ? 'is-invalid' : '' }}" type="text" name="timezone" id="timezone" value="{{ old('timezone', $notificationSetting->timezone) }}">
