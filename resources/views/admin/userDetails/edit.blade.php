    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.user-details.update", [$userDetail->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('facility') ? 'is-invalid' : '' }}" name="facility_id" id="facility_id">
                    @foreach($facilities as $id => $entry)
                        <option value="{{ $id }}" {{ (old('facility_id') ? old('facility_id') : $userDetail->facility->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('npi_num') ? 'is-invalid' : '' }}" type="text" name="npi_num" id="npi_num" value="{{ old('npi_num', $userDetail->npi_num) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('specialities') ? 'is-invalid' : '' }}" name="specialities[]" id="specialities" multiple>
                    @foreach($specialities as $id => $speciality)
                        <option value="{{ $id }}" {{ (in_array($id, old('specialities', [])) || $userDetail->specialities->contains($id)) ? 'selected' : '' }}>{{ $speciality }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('profile') ? 'is-invalid' : '' }}" name="profile_id" id="profile_id">
                    @foreach($profiles as $id => $entry)
                        <option value="{{ $id }}" {{ (old('profile_id') ? old('profile_id') : $userDetail->profile->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('lat') ? 'is-invalid' : '' }}" type="text" name="lat" id="lat" value="{{ old('lat', $userDetail->lat) }}">
                @if($errors->has('lat'))
                    <div class="invalid-feedback">
                        {{ $errors->first('lat') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('lng') ? 'is-invalid' : '' }}" type="text" name="lng" id="lng" value="{{ old('lng', $userDetail->lng) }}">
                @if($errors->has('lng'))
                    <div class="invalid-feedback">
                        {{ $errors->first('lng') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('experience') ? 'is-invalid' : '' }}" name="experience_id" id="experience_id">
                    @foreach($experiences as $id => $entry)
                        <option value="{{ $id }}" {{ (old('experience_id') ? old('experience_id') : $userDetail->experience->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
