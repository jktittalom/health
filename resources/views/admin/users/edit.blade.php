    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.users.update", [$user->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('device_type') ? 'is-invalid' : '' }}" type="text" name="device_type" id="device_type" value="{{ old('device_type', $user->device_type) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('device_token') ? 'is-invalid' : '' }}" type="text" name="device_token" id="device_token" value="{{ old('device_token', $user->device_token) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('mall_practice') ? 'is-invalid' : '' }}" name="mall_practice" id="mall_practice">
                    ...
                    @foreach(App\Models\User::MALL_PRACTICE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('mall_practice', $user->mall_practice) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('facilties') ? 'is-invalid' : '' }}" name="facilties[]" id="facilties" multiple required>
                    @foreach($facilties as $id => $facilty)
                        <option value="{{ $id }}" {{ (in_array($id, old('facilties', [])) || $user->facilties->contains($id)) ? 'selected' : '' }}>{{ $facilty }}</option>
                    @endforeach
                </select>
