...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.tracker.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.trackers.update", [$tracker->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('client') ? 'is-invalid' : '' }}" name="client_id" id="client_id" required>
                    @foreach($clients as $id => $entry)
                        <option value="{{ $id }}" {{ (old('client_id') ? old('client_id') : $tracker->client->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('contract') ? 'is-invalid' : '' }}" name="contract_id" id="contract_id" required>
                    @foreach($contracts as $id => $entry)
                        <option value="{{ $id }}" {{ (old('contract_id') ? old('contract_id') : $tracker->contract->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('view') ? 'is-invalid' : '' }}" type="number" name="view" id="view" value="{{ old('view', $tracker->view) }}" step="1">
                @if($errors->has('view'))
                    <div class="invalid-feedback">
                        {{ $errors->first('view') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('view_users') ? 'is-invalid' : '' }}" name="view_users" id="view_users">{{ old('view_users', $tracker->view_users) }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('click') ? 'is-invalid' : '' }}" type="number" name="click" id="click" value="{{ old('click', $tracker->click) }}" step="1">
                @if($errors->has('click'))
                    <div class="invalid-feedback">
                        {{ $errors->first('click') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('click_users') ? 'is-invalid' : '' }}" name="click_users" id="click_users">{{ old('click_users', $tracker->click_users) }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('applied') ? 'is-invalid' : '' }}" type="number" name="applied" id="applied" value="{{ old('applied', $tracker->applied) }}" step="1">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('applied_users') ? 'is-invalid' : '' }}" name="applied_users" id="applied_users">{{ old('applied_users', $tracker->applied_users) }}</textarea>
